...
export function Tooltip({
  ...
  const scheduleOpen = (nextOpen: boolean, delaySeconds: number) => {
    clearTimers();
    if (delaySeconds <= 0) { setOpen(nextOpen); return; }
    const timeoutId = window.setTimeout(() => setOpen(nextOpen), delaySeconds * 1000);
    if (nextOpen) openTimerRef.current = timeoutId;
    else closeTimerRef.current = timeoutId;
  };

  // Reposition popup after it's rendered
  useLayoutEffect(() => {
  ...
  const triggerProps =
    trigger === 'hover'
      ? {
          onMouseEnter: () => scheduleOpen(true, mouseEnterDelay),
          onMouseLeave: () => scheduleOpen(false, mouseLeaveDelay),
        }
      : trigger === 'focus'
  ...
  const tooltipStyle = {
    ...(color ? { ['--orot-tooltip-bg' as const]: color } : {}),
    ...style,
  };

  const shouldRender = open || !destroyOnHidden;

  const popup = shouldRender && title ? (
    <div
      ref={popupRef}
      role="tooltip"
      hidden={!open}
      className={[
        'orot-tooltip',
        `orot-tooltip--${placement}`,
        !arrow && 'orot-tooltip--no-arrow',
      ].filter(Boolean).join(' ')}
      style={{
        position: 'fixed',
  ...
  return (
    <>
      <span
        ref={triggerRef}
        className={['orot-tooltip-wrapper', className].filter(Boolean).join(' ')}
        {...triggerProps}
        {...rest}
      >
        {children}
